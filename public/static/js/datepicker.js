...
(function (factory) {
    ...
    Calendar.prototype.init = function(){
        ...
        //$('body').addClass('open');
        //$('.calendar-list span').removeClass('active');
    ...
    Calendar.prototype.createCalenList = function(data, setTitle){
        ...
        for(var i = 0 ; i < dSun ; i++){
            ...
            var n = i + 1,
                nDay =(tYear + '-' + extra(tMonth) + '-' + extra(n)),
                oSpan = create('span'),
                oNum = create('a', {
                    "data-calen" : nDay,
                    "href" : 'javascript:;'
            ...
            for(var x in orderDate){
                if(tYear==strY(orderDate[x])&&tMonth==strM(orderDate[x])&&n==strD(orderDate[x])){
                    oSpan.className = oSpan.className + ' youke';
                }
            }

            for(var x in searchDate){
                if(tYear==strY(searchDate[x])&&tMonth==strM(searchDate[x])&&n==strD(searchDate[x])){
                    oSpan.className = oSpan.className + ' select_day';
                }
            }

            if(dateDiff(nDay,toDay)<-14){
                oSpan.className = oSpan.className + ' expire pasted';
            }

            if(!data.m && !data.y || !data.y && $this.fixDate.m == tMonth){

                if(($this.fixDate.d == n && $this.fixDate.m == tMonth) || (!$this.fixDate.d && n == tDay)){
                    oSpan.className = oSpan.className + ' today';
                }else if((past || $this.hoursPast) && n < tDay){
                    oSpan.className = oSpan.className + ' expire pasted';
                }
            }
            else if((past || $this.hoursPast) && data.m < 0 && data.y <= 0){
                oSpan.className = ' expire pasted';
            }

            // 设置是否小于用户定义的开始日期
            if(tYear <= $this.fixDate.y && tMonth <= $this.fixDate.m && n < data.d || tYear <= $this.fixDate.y && tMonth < $this.fixDate.m){
                if($this.startDate)toolClass(oSpan, 'expire pasted');
            }

            // 设置禁用日期
            if(setShiled(tYear, tMonth, n))toolClass(oSpan, 'pasted shield');
    ...
    Calendar.prototype.createHeader = function(cb){
        ...
        //prevYear = create('a', {"class" : 'year-prev switch-btn icon-arrow-left', "href" : 'javascript:;'}, ''),
        //nextYear = create('a', {"class" : 'year-next switch-btn icon-arrow-right', "href" : 'javascript:;'}, ''),
        ...
        var month = create('div', {"class" : 'calendar-month'}),
            prevMonth = create('a', {"class" : 'month-prev switch-btn icon-arrow-left', "href" : 'javascript:;'}, ''),
            nextMonth = create('a', {"class" : 'month-next switch-btn icon-arrow-right', "href" : 'javascript:;'}, ''),
    ...
    Calendar.prototype.addEvent = function(){
        ...
        for(var i = 0 ; i < aCalenSet.length ; i++){
            aCalenSet[i].onclick = function(){
                if(toolClass(this, 'prev-to-month', 'has')){
                ...
                else if(!toolClass(this.parentNode, 'pasted', 'has') && !toolClass(this.parentNode, 'shield', 'has')){
                    ...
                    if($this.hours){
                        ...
                        //点击日期
                        searchTime = '';
                        //console.log(date.substring(0,10).trim());
                        var vdd = date.substring(0,10).trim();
                        if(searchDate.indexOf(vdd)==-1){
                            searchDate.push(vdd);
                        }else{
                            searchDate.remove(vdd);
                        }
                        // for (var i = 0; i < searchDate.length; i++) {
                        //     searchTime += searchDate[i] + ",";
                        // }
                        ...
                        $(this).parent().toggleClass('select_day');
                        //hideCalen();
                    }
                }
            }
    ...
    Calendar.prototype.transitions = function(obj, dir){
        ...
        function end(){
            ...
            $('.calendar-list span').removeClass();
...
function strY(e){
    var _m = new Date(e).getFullYear();
    return _m
}
function strM(e){
    var _m = new Date(e).getMonth() + 1;
    return _m
}
function strD(e){
    var _m = new Date(e).getDate();
    return _m
}
