...
class Wechat
{
    ...
    {
        $this->isqiye = isset($options['qiye'])?true:false;
    ...
    public function valid($return=false)
    {
        ...
        if (!$this->checkSignature($encryptStr) && $this->appid) {
            if ($return)
                return false;
            else
                die('no access');
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    public function checkAuth($appid='',$appsecret='',$token=''){
        ...
        if($this->isqiye){
            $result = $this->http_get("https://qyapi.weixin.qq.com/cgi-bin/gettoken?corpid={$appid}&corpsecret={$appsecret}");
        }else{
    ...
    public function getJsTicket($appid='',$jsapi_ticket=''){
        ...
        if($this->isqiye){
            $result = $this->http_get("https://qyapi.weixin.qq.com/cgi-bin/get_jsapi_ticket?access_token={$this->access_token}");
        }else {
     ...
     * @return string
     */
    public function getOauthRedirect($callback,$state='',$scope='snsapi_userinfo',$oauth=null){
        if(is_null($oauth)){
            return self::OAUTH_PREFIX.self::OAUTH_AUTHORIZE_URL.'appid='.$this->appid.'&redirect_uri='.urlencode($callback).'&response_type=code&scope='.$scope.'&state='.$state.'#wechat_redirect';
        }
        return $oauth.'?'.'appid='.$this->appid.'&redirect_uri='.urlencode($callback).'&response_type=code&scope='.$scope.'&state='.$state.'#wechat_redirect';
    }
    ...
    public function getUserId(){
        ...
        if (!$code) return false;
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_get("https://qyapi.weixin.qq.com/cgi-bin/user/getuserinfo?access_token={$this->access_token}&code={$code}");
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * @param string $customer_account 转发到指定客服帐号：test1@inquiry
     */
    ...
    "kf_online_list": [
    {
    "kf_account": "test1@inquiry",	//客服账号@微信别名
     ...
     *     "errcode" : 0,
     *     "errmsg" : "ok",
     *     "kf_account" : "test1@inquiry",    //正在接待的客服
